export default function Projects() {
  return (
    <section id="projects" className="px-[15%]">
      <h5>Projetos</h5>
      <div className="grid grid-cols-3 gap-10">
        <div className="projectCards">
          <img
            src="./print1.png"
            alt="Projeto1"
            className="h-[15rem] w-full rounded-t-xl object-cover"
          />
          <div className="flex flex-col space-y-5 px-5">
            <div className="space-y-5">
              <h5 className="text-xl font-bold">Organiza Lab</h5>
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Eaque
                vero quia architecto minus doloremque, id recusandae illum iure!
                Molestias incidunt ipsa reprehenderit! Ad eum maiores quo ipsum
                quod libero voluptas!
              </p>
            </div>
            <div className="flex space-x-5">
              <span className="linguagens">React</span>
              <span className="linguagens">laravel</span>
              <span className="linguagens"> Next</span>
            </div>
            <div className="flex space-x-2">
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-github-icon lucide-github"
                >
                  <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                  <path d="M9 18c-4.51 2-5-2-7-2" />
                </svg>
                Git
              </a>
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-external-link-icon lucide-external-link"
                >
                  <path d="M15 3h6v6" />
                  <path d="M10 14 21 3" />
                  <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                </svg>
                Site
              </a>
            </div>
          </div>
        </div>
        <div className="projectCards">
          <img
            src="./print1.png"
            alt="Projeto1"
            className="h-[15rem] w-full rounded-t-xl object-cover"
          />
          <div className="flex flex-col space-y-5 px-5">
            <div className="space-y-5">
              <h5 className="text-xl font-bold">Organiza Lab</h5>
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Eaque
                vero quia architecto minus doloremque, id recusandae illum iure!
                Molestias incidunt ipsa reprehenderit! Ad eum maiores quo ipsum
                quod libero voluptas!
              </p>
            </div>
            <div className="flex space-x-5">
              <span className="linguagens">React</span>
              <span className="linguagens">laravel</span>
              <span className="linguagens"> Next</span>
            </div>
            <div className="flex space-x-2">
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-github-icon lucide-github"
                >
                  <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                  <path d="M9 18c-4.51 2-5-2-7-2" />
                </svg>
                Git
              </a>
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-external-link-icon lucide-external-link"
                >
                  <path d="M15 3h6v6" />
                  <path d="M10 14 21 3" />
                  <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                </svg>
                Site
              </a>
            </div>
          </div>
        </div>
        <div className="projectCards">
          <img
            src="./print1.png"
            alt="Projeto1"
            className="h-[15rem] w-full rounded-t-xl object-cover"
          />
          <div className="flex flex-col space-y-5 px-5">
            <div className="space-y-5">
              <h5 className="text-xl font-bold">Organiza Lab</h5>
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Eaque
                vero quia architecto minus doloremque, id recusandae illum iure!
                Molestias incidunt ipsa reprehenderit! Ad eum maiores quo ipsum
                quod libero voluptas!
              </p>
            </div>
            <div className="flex space-x-5">
              <span className="linguagens">React</span>
              <span className="linguagens">laravel</span>
              <span className="linguagens"> Next</span>
            </div>
            <div className="flex space-x-2">
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-github-icon lucide-github"
                >
                  <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                  <path d="M9 18c-4.51 2-5-2-7-2" />
                </svg>
                Git
              </a>
              <a href="" className="flex space-x-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  className="lucide lucide-external-link-icon lucide-external-link"
                >
                  <path d="M15 3h6v6" />
                  <path d="M10 14 21 3" />
                  <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                </svg>
                Site
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
